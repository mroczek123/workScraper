...
import {
  JustJoinItJobOfferSimple,
  JustJoinItJobOfferSimpleResponse,
} from "./models";
  ...
  private getAllJobOffers(): Promise<Array<JustJoinItJobOfferSimple>> {
    return new Promise((resolve, reject) => {
      ...
      axios
        ...
        .then((response) =>
          response.data.map(
            (jobOfferSimple: JustJoinItJobOfferSimpleResponse) =>
              new JustJoinItJobOfferSimple(jobOfferSimple)
          )
        )
  ...
  getUrls(): Promise<Array<string>> {
    return new Promise((resolve, reject) => {
      this.getAllJobOffers()
        .then((data) =>
          data.map((jobOfferSimple) => this.getOfferDetailUrl(jobOfferSimple))
        )
        .then((data) => resolve(data));
    });
  }
